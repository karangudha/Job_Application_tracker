...
const Table = ({ jobs, getAllJobsData }) => {
    const formateCreateAtDistance = (d) => {
    ...
    const handleDelete = async (id) => {
        try {
            ...
            console.log(error);
            enqueueSnackbar(error.resopnse.data.message, {
                variant: "warning",
                anchorOrigin: {
                    vertical: "top",
                    horizontal: "right",
                    ...
                    <tbody>
                        {jobs?.map((item) => {
                            return (
                                <tr 
                                key={item._id}
                                className="odd:bg-balck-650 even:bg-gray-50 border-b"
                                >
                                    <th
                                        scope="row"
                                        className="px-6 py-4 fon-medium text-white-300 whitespace-nowrap"
                                    >
                                        {item.company}
                                    </th>
                                    ...
                                    } text-sm font-medium me-2 px-2.5 py-1 flex justify-center items-cener rounded`}>
                                        {item.status}
                                        </span>
                                    </td>
                                    <td className="px-6 py-4">
